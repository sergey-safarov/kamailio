...
#define trace_is_off(_msg) (((_msg)->msg_flags & FL_SIPTRACE) == 0)
...
static int w_sip_trace3(
		struct sip_msg *, char *dest, char *correlation_id, char *trace_type);
...
static void trace_tm_neg_ack_in(
...
static int mod_init(void)
{
	...
	*trace_on_flag = trace_on;
	...
	if(trace_dup_uri_str.s != 0) {
		...
		if(parse_uri(trace_dup_uri_str.s, trace_dup_uri_str.len, trace_dup_uri)
				< 0) {
	...
	if(trace_send_sock_name_str.s != 0) {
		trace_send_sock_info =
				ksr_get_socket_by_name(&trace_send_sock_name_str);
...
static int sip_trace_store(
		siptrace_data_t *sto, dest_info_t *dst, str *correlation_id_str)
{
	int ret = 1;
...
static int sip_trace_insert_db(
		db_key_t *db_keys, db_val_t *db_vals, int db_nkeys, char *dtext)
{
		...
		switch(sflags->s[idx] | 0x20) { /*|0x20 - to lowercase */
			...
			default:
				LM_ERR("Invalid character <%c> in <%.*s> at position <%d>!\n",
						sflags->s[idx], sflags->len, sflags->s, idx);
				return SIPTRACE_NONE;
		}
	}
		...
		str *corid, char *dir, enum siptrace_type_t trace_type)
{
	 ...
	 */
		...
		if(msg->first_line.type != SIP_REQUEST
				|| (trace_type == SIPTRACE_DIALOG
						&& msg->first_line.u.request.method_value
								   != METHOD_INVITE)) {
			LM_WARN("When tracing a %s sip_trace() has to be initiated on the "
					"%s\n",
					trace_type == SIPTRACE_TRANSACTION ? "transaction"
													   : "dialog",
					trace_type == SIPTRACE_TRANSACTION ? "request message"
													   : "initial invite");
			return -1;
		}
		...
		if(duri) {
			...
			info->u.dup_uri.s =
					p
					+ ((info->correlation_id.s) ? info->correlation_id.len : 0);
		...
		if(trace_type == SIPTRACE_TRANSACTION) {
			...
			if(unlikely(_siptrace_dlgb.set_dlg_var == NULL)) {
				...
				if(trace_add_info_xavp(info) < 0) {
					LM_ERR("failed to serialize siptrace info! Won't trace "
						   "dialog!\n");
					return -1;
				} else {
 ...
 */
static int ki_sip_trace_dst_cid_flag(
		sip_msg_t *msg, str *duri, str *cid, str *sflag)
{
	dest_info_t dst;
	enum siptrace_type_t trace_type;
	...
	return sip_trace_helper(
			msg, (duri) ? &dst : NULL, duri, cid, NULL, trace_type);
}
...
static int w_sip_trace3(
		sip_msg_t *msg, char *dest, char *correlation_id, char *trace_type_p)
{
	...
	if(dest || trace_to_database == 0) {
		if(dest) {
			if(fixup_get_svalue(msg, (gparam_t *)dest, &dup_uri_param_str)
					!= 0) {
		...
		if(dup_uri_param_str.s == 0 || (is_null_pv(dup_uri_param_str))) {
			if(trace_dup_uri_str.s == 0 || trace_dup_uri_str.len == 0) {
				LM_ERR("no duplicate_uri modparam nor duplicate uri "
	...
	if(correlation_id) {
		if(fixup_get_svalue(
				   msg, (gparam_t *)correlation_id, &correlation_id_str)
				!= 0) {
		...
		char *duri, char *corrid)
{
	...
	if(corrid != NULL
			&& fixup_get_svalue(msg, (gparam_t *)corrid, &corrid_str) != 0) {
		LM_ERR("unable to get correlation id parameter\n");
		return -1;
	}

	trace_send_hep_duplicate(&vmsg_str, &saddr_str, &taddr_str,
			(duri) ? &dest : NULL, (corrid) ? &corrid_str : NULL);
		...
		str *duri, str *corrid)
{
	...
	trace_send_hep_duplicate(vmsg, saddr, taddr,
			(duri && duri->len > 0) ? &dest : NULL,
...
static int sip_trace(
		sip_msg_t *msg, dest_info_t *dst, str *correlation_id_str, char *dir)
{
	siptrace_data_t sto;
...
static void trace_onreq_out(struct cell *t, int type, struct tmcb_params *ps)
{
	...
	if(info->uriState == STRACE_RAW_URI) {
		LM_BUG("uriState must be either UNUSED or PARSED here! must be a bug! "
			   "Message won't be traced!\n");
		return;
	}

	sip_trace_store(&sto,
			info->uriState == STRACE_PARSED_URI ? &info->u.dest_info : NULL,
			NULL);
	return;
}
...
static void trace_onreply_in(struct cell *t, int type, struct tmcb_params *ps)
{
	...
	sto.status.s =
			int2strbuf(ps->code, statusbuf, INT2STR_MAX_LEN, &sto.status.len);
	if(sto.status.s == 0) {
	...
	if(info->uriState == STRACE_RAW_URI) {
		LM_BUG("uriState must be either UNUSED or PARSED here - skip "
			   "tracing!\n");
		return;
	}
...
static void trace_sl_onreply_out(sl_cbp_t *slcbp)
{
	...
	sto.status.s = int2strbuf(
			slcbp->code, statusbuf, INT2STR_MAX_LEN, &sto.status.len);
	if(sto.status.s == 0) {
		LM_ERR("failure to get the status string\n");
		return;
	}
	...
	if(!xavp) {
		...
		LM_DBG("%.*s xavp not registered! "
			   "Probably incoming E2E CANCEL!\n",
				trace_xavp_info_name_s.len, trace_xavp_info_name_s.s);
		return;
	}
...
static int siptrace_exec_evcb_msg(siptrace_data_t *sto)
{
	...
	if(_siptrace_evrt_msg_idx >= 0) {
		...
		if(keng != NULL) {
			...
			(void)sr_kemi_route(
					keng, &msg, EVENT_ROUTE, &_siptrace_evcb_msg, &evname);
