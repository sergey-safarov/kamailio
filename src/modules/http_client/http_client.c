...
int prepare_keylog_file(str session_keylog_filename)
{
    ...
    if (session_keylog_filename.len == 0) {
        ...
        LM_DBG("Session keylog filename is empty, keylog disabled\n");
        return 0;
    }

    keylog_file = (char *)pkg_mallocxz(session_keylog_filename.len * sizeof(char) + 1);
    if(keylog_file == NULL) {
        PKG_MEM_ERROR;
        return -1;
    }
...
static int mod_init(void)
{
    str keylog_file_path = STR_STATIC_INIT("/var/lib/kamailio/http_keylog");
    ...
    prepare_keylog_file(keylog_file_path);
