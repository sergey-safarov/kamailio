...
static int ws_receive_sip_msg(char *buf, unsigned int len, tcp_event_info_t *tcpinfo)
{
	...
	switch(opcode) {
		case OPCODE_CONTINUATION:
			if(likely(frame.wsc->sub_protocol == SUB_PROTOCOL_SIP)) {
				...
				if(frame.fin) {
					ret = ws_receive_sip_msg(
							frame.wsc->frag_buf.s, frame.wsc->frag_buf.len, tcpinfo);
					wsconn_put(frame.wsc);
					return ret;
				}
				wsconn_put(frame.wsc);
