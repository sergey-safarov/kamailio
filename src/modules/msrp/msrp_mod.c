...
static int child_init(int rank)
{
	...
	if(msrp_cmap_size > 0) {
		if(fork_sync_timer(PROC_TIMER, "MSRP Timer", 1 /*socks flag*/,
				   msrp_local_timer, NULL, msrp_timer_interval /*sec*/)
				< 0) {
 ...
 *
 */
static int ki_msrp_reply(sip_msg_t *msg, str *rcode, str *rtext, str *rhdrs)
{
	msrp_frame_t *mf;
	int ret;
	...
	ret = msrp_reply(
			mf, rcode, rtext, (rhdrs != NULL && rhdrs->len > 0) ? rhdrs : NULL);
	if(ret == 0)
		ret = 1;
	return ret;
}
